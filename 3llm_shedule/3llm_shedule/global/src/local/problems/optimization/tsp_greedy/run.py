    ...
    def greedy(self, eva):
        ...
        for instance, distance_matrix in self.instance_data:
            ...
            for i in range(1, self.problem_size - 1):
                ...
                unvisited_near_size = np.minimum(
                    self.neighbor_size, unvisited_near_nodes.size
                )

                unvisited_near_nodes = unvisited_near_nodes[:unvisited_near_size]

                next_node = eva.select_next_node(
                    current_node,
                    destination_node,
            ...
            mask = ~np.isin(
                np.arange(self.problem_size), route[: self.problem_size - 1]
            )
